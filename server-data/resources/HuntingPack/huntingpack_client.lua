    ...
    while true 
        ...
        if ourTeamType == 'driver' then
            ...
            if lastVehicle == 'Firetruk' then
                SetVehicleCheatPowerIncrease(ourDriverVehicle, 0.75)
...
Citizen.CreateThread(function()
	...
	while true do
        ...
        if ourTeamType == 'driver' then 
            local speedinKMH = GetEntitySpeed(PlayerPedId()) * 3.6
            print(speedinKMH)
            if speedinKMH < MinSpeedInKMH and (GetGameTimer() - startTime)/1000 > 15 then
                timeBelowSpeed = (GetGameTimer() - timestart)/1000
                if shouldNotifyBelowSpeed then
            ...
            if ourTeamType == 'driver' then
                ...
                if (GetGameTimer() - startTime)/1000 < 15 then
                    ...
                    if timeBelowSpeed > 0 then
                        AddTextComponentString(("%.3f"):format(maxTimeBelowSpeed -timeBelowSpeed))
                        DrawText(0.5,  0.4)
                    end
                end
            else 
                if 15 - (GetGameTimer() - startTime)/1000 > 0 and totalLife < 15 then
                    SetTextFont(1)
                    SetTextProportional(0)
                    SetTextScale(0.0, 1.0)
                    SetTextColour(255, 0, 0, 255)
...
AddEventHandler('onHuntingPackStart', function(teamtype, spawnPos, spawnRot, driver)
    ...
    if total_players <= 2 then
        possibleDriverVehicles = {'stretch'}
...
Citizen.CreateThread(function()
    while true do
        ...
        if not gameStarted or (GetGameTimer() - startTime)/1000 < 15 then 
            DrawRect(0.8, 0.265, 0.4, 0.07, 252, 10, 12, 200)
            DrawRect(0.8, 0.525, 0.4, 0.45, 42, 42, 43, 150)
            SetTextFont(1)
            SetTextProportional(0)
            SetTextScale(0.0, 1.0)
            ...
            SetTextEntry("STRING")
            SetTextCentre(1)
            AddTextComponentString("Leaderboard")
            DrawText(0.80,  0.235)
            DrawPlayers()
        end
    end
...
function DrawPlayers()
    for _,player in pairs(ranks) do
        if player.points ~= 0 then
            ...
            DrawText(0.85,  0.2685+Yoffset*player.rank)
            SetTextFont(0)
            SetTextProportional(0)
            SetTextScale(0.0, 0.3)
            if player.rank == 1 then
